...
class NBFNet_KG(nn.Module):

    def __init__(self, input_dim, hidden_dims, num_relation, message_func="distmult", aggregate_func="pna",
                 aggregate_kwargs = None, short_cut=False, layer_norm=False, activation="relu", concat_hidden=False, num_mlp_layer=2,
        ...
        for i in range(len(self.dims) - 1):
            self.layers.append(GeneralizedRelationalConv(self.dims[i], self.dims[i + 1], num_relation,
                                                                self.dims[0], message_func, aggregate_func, aggregate_kwargs, layer_norm,
    ...
    def remove_easy_edges(self, data, h_index, t_index, r_index=None):
        ...
        if self.remove_one_hop:
            ...
            index = self.edge_match(edge_index, easy_edge)[0]
